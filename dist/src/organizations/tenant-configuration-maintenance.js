...
exports.tenantConfigurationPreflight = tenantConfigurationPreflight;
exports.tenantConfigurationBackfill = tenantConfigurationBackfill;
exports.tenantConfigurationValidate = tenantConfigurationValidate;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
async function tenantConfigurationPreflight(db, organizationId) {
    const [organizations, duplicateCodes, settings, branding, domains, files, platformAdmins, activeMemberships,] = await Promise.all([
        db.organization.groupBy({ by: ["status"], _count: { _all: true } }),
        db.$queryRaw(client_1.Prisma.sql `SELECT lower(trim("code")) AS code, count(*) AS count FROM "organizations" GROUP BY lower(trim("code")) HAVING count(*) > 1`),
    ...
    const target = organizationId
        ...
        : null;
    return {
        status: duplicateCodes.length || (organizationId && !target)
            ? "blocked"
            : "ready",
        organizations,
...
async function tenantConfigurationBackfill(db, organizationId, confirmApply) {
    ...
    if (!wouldCreate)
        return { status: "unchanged", created: 0, organizationId };
...
async function tenantConfigurationValidate(db) {
    const invalidSettings = await db.organizationSettings.count({
        where: {
            OR: [{ firstDayOfWeek: { lt: 0 } }, { firstDayOfWeek: { gt: 6 } }],
        },
    });
    const orphanDomains = await db.$queryRaw(client_1.Prisma.sql `SELECT count(*) AS count FROM "organization_domains" d LEFT JOIN "organizations" o ON o.id=d."organizationId" WHERE o.id IS NULL`);
    return {
        status: invalidSettings === 0 && Number(orphanDomains[0]?.count ?? 0) === 0
            ? "passed"
            : "failed",
        invalidSettings,
...
async function main() {
    ...
    const result = command === "preflight"
        ...
        : command === "backfill"
            ? await tenantConfigurationBackfill(prisma, organizationId, process.argv.includes("--confirm-apply"))
            : command === "validate"
...
//# sourceMappingURL=tenant-configuration-maintenance.js.map