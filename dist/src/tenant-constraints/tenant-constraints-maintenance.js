...
exports.inspectTenantConstraints = inspectTenantConstraints;
exports.backfillTenantConstraints = backfillTenantConstraints;
exports.validateTenantConstraints = validateTenantConstraints;
...
function normalizeDuplicates(rows) {
    return rows.map((row) => ({ ...row, count: Number(row.count) }));
}
async function inspectTenantConstraints(prisma) {
    const membershipTable = await prisma.$queryRaw(client_1.Prisma.sql `
  ...
  `);
    ...
    const [total, missing, invalidOrganizations, crossTenantManagers, crossTenantMemberships, duplicateGlobalCodes, duplicateTenantCodes, indexes,] = await Promise.all([
        prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(*) AS count FROM "teams"`),
        prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(*) AS count FROM "teams" WHERE "organizationId" IS NULL`),
        prisma.$queryRaw(client_1.Prisma.sql `
    ...
    if (missingCount)
        blockingConflicts.push(`teams missing organizationId: ${missingCount}`);
    if (invalidCount)
        blockingConflicts.push(`teams with invalid Organization reference: ${invalidCount}`);
    if (managerConflictCount)
        blockingConflicts.push(`cross-Tenant Team managers: ${managerConflictCount}`);
    if (membershipConflictCount)
        blockingConflicts.push(`cross-Tenant Membership teams: ${membershipConflictCount}`);
    if (tenantDuplicates.length)
        blockingConflicts.push(`duplicate Tenant-scoped Team codes: ${tenantDuplicates.length}`);
    return {
...
async function backfillTenantConstraints(prisma, options) {
...
async function validateTenantConstraints(prisma) {
    const report = await inspectTenantConstraints(prisma);
    return {
        ...report,
        status: report.blockingConflicts.length ? 'failed' : 'passed',
    };
}
async function main() {
...
if (require.main === module)
    void main();
//# sourceMappingURL=tenant-constraints-maintenance.js.map