...
exports.buildMembershipPlan = buildMembershipPlan;
exports.preflightMemberships = preflightMemberships;
exports.backfillMemberships = backfillMemberships;
exports.validateMemberships = validateMemberships;
const client_1 = require("@prisma/client");
const DEFAULT_BATCH_SIZE = 100;
async function membershipTableExists(prisma) {
...
async function candidates(prisma) {
...
async function buildMembershipPlan(prisma) {
    ...
    const existing = hasTable
        ...
        : [];
    const byUserOrganization = new Map(existing.map((item) => [`${item.userId}:${item.organizationId}`, item]));
    const defaultsByUser = new Map();
    for (const item of existing) {
        if (item.isDefault && item.status === client_1.OrganizationMembershipStatus.ACTIVE) {
    ...
    for (const user of users) {
        if (!user.organization)
            conflicts.push({ userId: user.id, code: 'INVALID_ORGANIZATION' });
        if (user.roleId && !user.assignedRole)
            conflicts.push({ userId: user.id, code: 'INVALID_ROLE' });
        if (user.teamId && !user.teamRef)
            conflicts.push({ userId: user.id, code: 'INVALID_TEAM' });
        ...
        const correct = current.status === client_1.OrganizationMembershipStatus.ACTIVE &&
            current.isDefault &&
        ...
        if (correct)
            ...
            conflicts.push({ userId: user.id, code: 'EXISTING_MEMBERSHIP_CONFLICT' });
    }
    return { candidates: users, existing, conflicts, createUserIds, unchangedUserIds };
}
async function preflightMemberships(prisma) {
...
async function backfillMemberships(prisma, options) {
...
async function validateMemberships(prisma) {
    const plan = await buildMembershipPlan(prisma);
    const invalidNonActiveDefaults = await prisma.organizationMembership.count({
        where: { isDefault: true, status: { not: client_1.OrganizationMembershipStatus.ACTIVE } },
    });
    const activeDefaultGroups = await prisma.organizationMembership.groupBy({
        by: ['userId'],
        where: { isDefault: true, status: client_1.OrganizationMembershipStatus.ACTIVE },
        _count: { _all: true },
        having: { userId: { _count: { gt: 1 } } },
    });
    const valid = plan.conflicts.length === 0 &&
...
//# sourceMappingURL=membership-maintenance.js.map