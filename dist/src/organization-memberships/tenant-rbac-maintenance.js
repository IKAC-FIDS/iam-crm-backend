...
exports.TenantRbacMaintenance = void 0;
const client_1 = require("@prisma/client");
class TenantRbacMaintenance {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async plan(organizationId) {
        ...
        for (const membership of memberships)
            membershipByUser.set(membership.userId, [...(membershipByUser.get(membership.userId) ?? []), membership]);
        const conflicts = [];
        const assignments = [];
        for (const user of users) {
            const candidates = membershipByUser.get(user.id) ?? [];
            if (candidates.length !== 1) {
                conflicts.push({ userId: user.id, code: candidates.length ? 'MULTIPLE_TARGET_MEMBERSHIPS' : 'NO_TARGET_MEMBERSHIP' });
                continue;
            }
            ...
            if (!role && !user.roleId) {
                ...
                if (matches.length === 1)
                    ...
                    conflicts.push({ userId: user.id, code: 'AMBIGUOUS_LEGACY_ENUM_ROLE' });
            }
            if (!role) {
                conflicts.push({ userId: user.id, code: 'INVALID_ROLE_REFERENCE' });
                continue;
            }
            if (role.scope === client_1.RoleScope.TENANT && role.organizationId !== organizationId) {
                conflicts.push({ userId: user.id, code: 'CROSS_TENANT_ROLE' });
                continue;
            }
        ...
        return {
            ...
            roles: roles.length, systemRoles: roles.filter((role) => role.scope === client_1.RoleScope.SYSTEM).length,
            tenantRoles: roles.filter((role) => role.scope === client_1.RoleScope.TENANT).length,
    ...
    async backfill(organizationId, apply) {
        ...
        if (plan.conflicts.length || plan.permissionMismatches.length)
            throw new Error(`Backfill blocked: ${plan.conflicts.length} conflicts, ${plan.permissionMismatches.length} permission mismatches`);
        if (!apply)
            return { mode: 'dry-run', ...this.publicReport(plan) };
        await this.prisma.$transaction(async (tx) => {
            let changed = 0;
            for (const assignment of plan.assignments)
                changed += (await tx.organizationMembership.updateMany({ where: { id: assignment.membershipId, organizationId, roleId: null }, data: { roleId: assignment.roleId } })).count;
            if (changed) {
    ...
    async validate(organizationId) {
        const plan = await this.plan(organizationId);
        const invalidScopes = await this.prisma.role.count({ where: { OR: [{ scope: client_1.RoleScope.SYSTEM, organizationId: { not: null } }, { scope: client_1.RoleScope.TENANT, organizationId: null }] } });
...
exports.TenantRbacMaintenance = TenantRbacMaintenance;
...
async function main() {
    ...
    if (!organizationId)
        throw new Error('--organization <uuid> is required; broad backfill is intentionally unsupported');
    ...
    try {
        ...
        const { assignments: _assignments, ...safe } = result;
...
//# sourceMappingURL=tenant-rbac-maintenance.js.map