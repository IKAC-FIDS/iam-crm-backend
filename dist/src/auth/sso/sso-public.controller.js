...
let SsoPublicController = class SsoPublicController {
    ...
    listProviders(domain, subdomain) {
        if (Boolean(domain) === Boolean(subdomain))
            throw new common_1.BadRequestException("Exactly one verified tenant route is required");
        return this.service.discoverPublicProviders(domain ? "DOMAIN" : "SUBDOMAIN", domain ?? subdomain);
    }
};
