...
exports.ssoTenantPreflight = ssoTenantPreflight;
exports.ssoTenantBackfill = ssoTenantBackfill;
exports.ssoTenantValidate = ssoTenantValidate;
const client_1 = require("@prisma/client");
const normalizeName = (value) => value.trim().toLocaleLowerCase("en-US");
const normalizeDomain = (value) => value
    .trim()
    .toLowerCase()
    .replace(/^\.+|\.+$/g, "");
async function tableReady(prisma) {
...
async function ssoTenantPreflight(prisma, organizationId) {
    ...
    const duplicate = (key) => Object.entries(proposed.reduce((out, row) => {
        const value = row[key];
        if (value)
    ...
    const domains = Object.entries(proposed
    ...
    const incompatibleSecrets = providers.filter((p) => p.clientSecretEnc && !p.clientSecretEnc.startsWith("gcm:")).length;
    const blocked = !target ||
        target.status !== client_1.OrganizationStatus.ACTIVE ||
    ...
    return {
        ...
        providersWithoutOrganization: providers.filter((p) => !p.organizationId)
            .length,
        externalIdentities: providers.reduce((sum, p) => sum + p.externalIdentities.length, 0),
        orphanIdentities,
        encryptedSecrets: providers.filter((p) => p.clientSecretEnc?.startsWith("gcm:")).length,
        incompatibleSecrets,
        conflicts,
        proposed,
    };
}
async function ssoTenantBackfill(prisma, organizationId, confirmApply) {
    ...
    if (!confirmApply)
        return {
            ...
            routesToCreate: pending.reduce((sum, p) => sum + new Set(p.allowedDomains.map(normalizeDomain)).size, 0),
        };
    return prisma.$transaction(async (tx) => {
        ...
        for (const provider of pending) {
            ...
            const routes = [
                ...new Set(provider.allowedDomains.map(normalizeDomain).filter(Boolean)),
            ];
            if (routes.length)
                routesCreated += (await tx.ssoProviderRoute.createMany({
...
async function ssoTenantValidate(prisma) {
    const [providers, withoutOrganization, withoutNormalizedName, identityCount, orphanIdentities,] = await Promise.all([
        ...
        prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(*) AS count FROM external_identities e LEFT JOIN sso_providers p ON p.id=e."providerId" LEFT JOIN users u ON u.id=e."userId" WHERE p.id IS NULL OR u.id IS NULL`),
    ]);
    return {
        status: withoutOrganization === 0 &&
...
async function main() {
    ...
    try {
        const result = command === "preflight"
            ...
            : command === "backfill"
                ? await ssoTenantBackfill(prisma, organizationId, args.includes("--confirm-apply"))
                : command === "validate"
...
if (require.main === module)
    main().catch((error) => {
        console.error(error instanceof Error ? error.message : "SSO tenant maintenance failed");
        process.exitCode = 1;
    });
//# sourceMappingURL=sso-tenant-maintenance.js.map