...
let AuditLogService = class AuditLogService {
    constructor(prisma, requestContext, exporter) {
        this.prisma = prisma;
    ...
    async findAll(query, user) {
        const page = query.page ?? 1, limit = query.limit ?? 20, where = this.where(query, user);
    ...
    async summary(query, user) {
        ...
        const count = (key) => [...this.group(rows, (r) => r[key])]
        ...
        const byActor = [...this.group(rows, (r) => r.actorId ?? "SYSTEM")]
            ...
            actorName: id === "SYSTEM" ? "System" : (actors.get(id)?.fullName ?? "Unknown"),
            count: items.length,
        }))
            .sort((a, b) => b.count - a.count);
        return {
    ...
    async filterOptions(query, user) {
        const rows = await this.prisma.auditLog.findMany({
    ...
    async export(query, user) {
        const format = query.format ?? "csv", max = query.includePayload ? 5000 : format === "csv" ? 50000 : 20000, where = this.where(query, user), total = await this.prisma.auditLog.count({ where });
        if (total > max)
    ...
    trend(rows) {
        if (!rows.length)
            return [];
        const min = Math.min(...rows.map((r) => r.createdAt.getTime())), max = Math.max(...rows.map((r) => r.createdAt.getTime())) + 1, step = max - min <= 31 * 86400000 ? 86400000 : 7 * 86400000, out = [];
        for (let t = min; t < max; t += step) {
            const end = Math.min(max, t + step);
            out.push({
                periodStart: new Date(t).toISOString(),
                periodEnd: new Date(end).toISOString(),
                count: rows.filter((r) => r.createdAt.getTime() >= t && r.createdAt.getTime() < end).length,
            });
        }
        return out;
    }
    ...
    period(q) {
        const r = (0, api_date_util_1.parseApiDateRange)(q.startDate, q.endDate);
        return {
