...
async function backfill(apply) {
    ...
    if (!apply)
        return { mode: "dry-run", candidates: before.incomplete, ...before };
    if (!confirmed)
        throw new Error("Backfill apply requires --confirm-apply");
    const changed = await prisma.$executeRaw(client_1.Prisma.sql `
    UPDATE "audit_logs"
...
async function archive(apply) {
  ...
  `);
    ...
    if (!apply)
        return { mode: "dry-run", cutoff: cutoff.toISOString(), candidates, sourceRowsWillBeDeleted: false };
    if (!confirmed)
        throw new Error("Archive apply requires --confirm-apply");
    const batchId = (0, node_crypto_1.randomUUID)();
    const copied = await prisma.$executeRaw(client_1.Prisma.sql `
  ...
  `);
    ...
    if (copied !== archived)
        throw new Error(`Archive verification failed: copied=${copied} verified=${archived}`);
    return { mode: "apply-copy-only", cutoff: cutoff.toISOString(), batchId, copied, verified: archived, sourceRowsDeleted: 0 };
}
async function main() {
    let result;
    if (command === "preflight")
        result = { status: "READY", ...(await counts()) };
    else if (command === "backfill")
        result = await backfill(confirmed);
    else if (command === "validate") {
        const state = await counts();
        if (state.incomplete || state.invalidDuration)
            throw new Error(`Audit validation failed: ${JSON.stringify(state)}`);
        result = { status: "VALID", ...state };
    }
    else if (command === "archive")
        result = await archive(confirmed);
        ...
        throw new Error("Use preflight, backfill, validate, archive, or retention");
...
//# sourceMappingURL=audit-log-maintenance.js.map