...
let SmsNotificationChannelHandler = SmsNotificationChannelHandler_1 = class SmsNotificationChannelHandler {
    ...
    async dispatch(deliveryId) {
        const claimed = await this.prisma.notificationDelivery.updateMany({
            where: { id: deliveryId, channel: client_1.NotificationChannel.SMS, status: { in: [client_1.NotificationDeliveryStatus.PENDING, client_1.NotificationDeliveryStatus.RETRYING] } },
        ...
        if (!delivery?.recipientUser || !delivery.template)
            return this.skip(deliveryId, "INCOMPLETE_DELIVERY", "گیرنده یا قالب پیامک موجود نیست");
        const destination = await this.contacts.resolve(delivery.event.organizationId, delivery.recipientUser.id);
        if (!destination)
            return this.skip(deliveryId, "MISSING_SMS_DESTINATION", "شماره موبایل معتبر برای گیرنده یافت نشد");
        try {
            ...
            if (!result.success) {
                await this.prisma.notificationDelivery.update({ where: { id: deliveryId }, data: { status: client_1.NotificationDeliveryStatus.FAILED, destination, failureCode: result.errorCode ?? "SMS_PROVIDER_FAILED", failureMessage: result.errorMessage?.slice(0, 1000) ?? "ارسال پیامک ناموفق بود" } });
            ...
            await this.prisma.notificationDelivery.update({ where: { id: deliveryId }, data: { status: client_1.NotificationDeliveryStatus.SENT, destination, providerMessageId: result.providerMessageId ?? null, failureCode: null, failureMessage: null, sentAt: new Date() } });
            ...
            await this.prisma.notificationDelivery.update({ where: { id: deliveryId }, data: { status: client_1.NotificationDeliveryStatus.FAILED, destination, failureCode: "SMS_DISPATCH_ERROR", failureMessage: message } });
            return { deliveryId, status: client_1.NotificationDeliveryStatus.FAILED, sent: false, reason: "SMS_DISPATCH_ERROR" };
        }
    }
    async skip(deliveryId, code, message) {
        await this.prisma.notificationDelivery.update({ where: { id: deliveryId }, data: { status: client_1.NotificationDeliveryStatus.SKIPPED, failureCode: code, failureMessage: message } });
