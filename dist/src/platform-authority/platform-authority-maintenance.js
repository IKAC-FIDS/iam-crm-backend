...
exports.preflightPlatformAuthority = preflightPlatformAuthority;
exports.grantPlatformAuthority = grantPlatformAuthority;
exports.revokePlatformAuthority = revokePlatformAuthority;
exports.validatePlatformAuthority = validatePlatformAuthority;
...
async function exactUser(prisma, userId) {
    return prisma.user.findUnique({
        where: { id: userId },
        select: { id: true, email: true, fullName: true, isActive: true },
    });
}
async function preflightPlatformAuthority(prisma, userId) {
    const exists = await tableExists(prisma);
    const [users, activeUsers, roleDistribution, authorities, inactiveAuthorities, duplicateAssignments, auditColumns, target,] = await Promise.all([
        prisma.user.count(),
    ...
    const auditOrganizationIdNullable = auditColumns[0]?.organizationIdNullable === true;
    return {
        status: (userId && (!target || !target.isActive)) ||
...
async function grantPlatformAuthority(prisma, userId, confirmApply) {
    if (!confirmApply)
        throw new Error('Grant requires --confirm-apply');
    if (!(await tableExists(prisma)))
        throw new Error('platform_authorities table does not exist');
    return prisma.$transaction(async (tx) => {
        ...
        const inserted = await tx.platformAuthority.createMany({
            data: [{ userId, role: client_1.PlatformRole.PLATFORM_ADMIN }],
            skipDuplicates: true,
        });
...
async function revokePlatformAuthority(prisma, userId, confirmApply) {
    if (!confirmApply)
        throw new Error('Revoke requires --confirm-apply');
    if (!(await tableExists(prisma)))
        throw new Error('platform_authorities table does not exist');
    return prisma.$transaction(async (tx) => {
        const user = await exactUser(tx, userId);
        ...
        if (removed.count) {
            await tx.auditLog.create({
                data: {
                    ...
                    metadata: { targetUserId: userId, role: client_1.PlatformRole.PLATFORM_ADMIN, source: 'operator-cli' },
                },
            });
        }
...
async function validatePlatformAuthority(prisma) {
    const exists = await tableExists(prisma);
    if (!exists)
        return { status: 'failed', tableExists: false };
...
//# sourceMappingURL=platform-authority-maintenance.js.map