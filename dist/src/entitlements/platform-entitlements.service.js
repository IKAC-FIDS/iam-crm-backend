...
let PlatformEntitlementsService = class PlatformEntitlementsService {
    ...
    audit(tx, platform, action, entityId, before, after, metadata) { return tx.auditLog.create({ data: { actorId: platform.userId, actorType: 'PLATFORM_ADMIN', scope: 'PLATFORM', source: 'PLATFORM', result: 'SUCCESS', organizationId: null, entityType: 'commercial-entitlement', entityId, action, before: before ?? undefined, after: after ?? undefined, metadata: { platformRole: platform.platformRole, ...metadata }, requestId: platform.requestId ?? null } }); }
