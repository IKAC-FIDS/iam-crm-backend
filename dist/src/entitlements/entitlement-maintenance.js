...
class EntitlementMaintenance {
    ...
    async preflight(organizationId) {
        ...
        const currentStatuses = [client_1.SubscriptionStatus.PENDING, client_1.SubscriptionStatus.ACTIVE, client_1.SubscriptionStatus.SUSPENDED];
        for (const item of subscriptions.filter((row) => currentStatuses.includes(row.status)))
            currentByOrganization.set(item.organizationId, (currentByOrganization.get(item.organizationId) ?? 0) + 1);
    ...
    async bootstrap(apply) {
        ...
        if (!apply)
            return { mode: 'dry-run', plansToCreate: missing, featureMatrix: 'UNASSIGNED_REQUIRES_COMMERCIAL_APPROVAL' };
        if (missing.length)
            await this.prisma.$transaction(async (tx) => { for (const item of missing) {
                const plan = await tx.plan.create({ data: item });
                await tx.auditLog.create({ data: { organizationId: null, entityType: 'commercial-entitlement', entityId: plan.id, action: 'plan.bootstrap-created', after: item } });
            } });
        return { mode: 'apply', created: missing.length, unchanged: existing.length, featureMatrix: 'UNASSIGNED_REQUIRES_COMMERCIAL_APPROVAL' };
    }
    async backfill(organizationId, planId, apply) {
        const [organization, plan, current] = await Promise.all([this.prisma.organization.findUnique({ where: { id: organizationId }, select: { id: true } }), this.prisma.plan.findFirst({ where: { id: planId, isActive: true }, select: { id: true, code: true, _count: { select: { features: true } } } }), this.prisma.subscription.findFirst({ where: { organizationId, status: { in: [client_1.SubscriptionStatus.PENDING, client_1.SubscriptionStatus.ACTIVE, client_1.SubscriptionStatus.SUSPENDED] } } })]);
        if (!organization)
            throw new Error('Exact target Organization does not exist');
        if (!plan)
            throw new Error('Exact active Plan does not exist');
        if (plan._count.features !== Object.keys(client_1.FeatureKey).length)
            throw new Error('Plan Feature matrix is incomplete; obtain Commercial approval and configure every Feature explicitly');
        if (current)
            return { mode: apply ? 'apply' : 'dry-run', created: 0, unchanged: 1, subscriptionId: current.id };
        if (!apply)
            return { mode: 'dry-run', created: 1, organizationId, planId, type: client_1.SubscriptionType.MANUAL_CONTRACT, status: client_1.SubscriptionStatus.ACTIVE, openEnded: true };
        const result = await this.prisma.$transaction(async (tx) => {
            await tx.$queryRaw(client_1.Prisma.sql `SELECT CAST(pg_advisory_xact_lock(hashtext(${'subscription:' + organizationId})) AS TEXT) AS "lockResult"`);
            const existing = await tx.subscription.findFirst({ where: { organizationId, status: { in: [client_1.SubscriptionStatus.PENDING, client_1.SubscriptionStatus.ACTIVE, client_1.SubscriptionStatus.SUSPENDED] } } });
            if (existing)
                return existing;
            const subscription = await tx.subscription.create({ data: { organizationId, planId, type: client_1.SubscriptionType.MANUAL_CONTRACT, status: client_1.SubscriptionStatus.ACTIVE, startAt: new Date(0), contractReference: 'FIX-000092-EXPLICIT-COMPATIBILITY' } });
            await tx.organization.update({ where: { id: organizationId }, data: { entitlementVersion: { increment: 1 } } });
            await tx.auditLog.create({ data: { organizationId: null, entityType: 'commercial-entitlement', entityId: subscription.id, action: 'subscription.compatibility-backfilled', metadata: { targetOrganizationId: organizationId, planId } } });
            return subscription;
        });
        return { mode: 'apply', created: 1, subscriptionId: result.id };
    }
    async validate(organizationId) { const report = await this.preflight(organizationId); const rows = await this.prisma.subscription.findMany({ where: organizationId ? { organizationId } : {}, select: { id: true, startAt: true, endAt: true, gracePeriodEndAt: true } }); const invalid = rows.filter((row) => (row.endAt && row.endAt <= row.startAt) || (row.gracePeriodEndAt && (!row.endAt || row.gracePeriodEndAt <= row.endAt))).map((row) => row.id); return { valid: report.ambiguousCurrentSubscriptions.length === 0 && invalid.length === 0, invalidSubscriptionIds: invalid, ...report }; }
}
exports.EntitlementMaintenance = EntitlementMaintenance;
function arg(name) { const i = process.argv.indexOf(name); return i >= 0 ? process.argv[i + 1] : undefined; }
async function main() { const prisma = new client_1.PrismaClient(); try {
    const tool = new EntitlementMaintenance(prisma), command = process.argv[2], organizationId = arg('--organization'), planId = arg('--plan');
    let result;
    if (command === 'preflight')
        result = await tool.preflight(organizationId);
    else if (command === 'bootstrap')
        result = await tool.bootstrap(process.argv.includes('--confirm-apply'));
    else if (command === 'backfill') {
        if (!organizationId || !planId)
            throw new Error('--organization and --plan exact UUIDs are required');
        result = await tool.backfill(organizationId, planId, process.argv.includes('--confirm-apply'));
    }
    else if (command === 'validate')
        result = await tool.validate(organizationId);
    else
        throw new Error('Expected preflight, bootstrap, backfill, or validate');
...
//# sourceMappingURL=entitlement-maintenance.js.map