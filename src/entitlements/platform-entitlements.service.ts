...
export class PlatformEntitlementsService {
  ...
  private audit(tx: Prisma.TransactionClient, platform: PlatformScopeContext, action: string, entityId: string, before: unknown, after: unknown, metadata?: Record<string, unknown>) { return tx.auditLog.create({ data: { actorId: platform.userId, organizationId: null, entityType: 'commercial-entitlement', entityId, action, before: before as Prisma.InputJsonValue ?? undefined, after: after as Prisma.InputJsonValue ?? undefined, metadata: { platformRole: platform.platformRole, ...metadata } as Prisma.InputJsonValue, requestId: platform.requestId ?? null } }); }
