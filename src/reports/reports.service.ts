...
import { ActivityType, PipelineStage, Prisma, Priority, UserRole } from '@prisma/client';
...
export class ReportsService {
  ...
  async getPipelineByOwner(filters: ReportFiltersDto, user: CurrentUserPayload) {
    ...
    return [...owners.entries()].map(([ownerId, items]) => {
      const doneCompanies = items.filter((item) => item.stage === PipelineStage.DONE).length;
  ...
  private isLostStage(stage: PipelineStage) {
