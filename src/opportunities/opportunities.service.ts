...
export class OpportunitiesService {
  ...
  findByCompany(companyId: string, query: FindOpportunitiesDto, user: CurrentUserPayload) {
  ...
  async changeOwner(id: string, dto: ChangeOpportunityOwnerDto, user: CurrentUserPayload) {
    const current = await this.getForMutation(id, user);
    if (dto.ownerId) await this.validateOwner(dto.ownerId, user);
  ...
  async archive(id: string, dto: ArchiveOpportunityDto, user: CurrentUserPayload) {
    const current = await this.getForMutation(id, user);
    if (current.archivedAt) throw new BadRequestException('Opportunity is already archived');
  ...
  async restore(id: string, user: CurrentUserPayload) {
    const current = await this.getForMutation(id, user);
    if (!current.archivedAt) throw new BadRequestException('Opportunity is not archived');
  ...
  private buildWhere(query: FindOpportunitiesDto, user: CurrentUserPayload): Prisma.OpportunityWhereInput {
    const and: Prisma.OpportunityWhereInput[] = [this.scopeWhere(user), { company: { archivedAt: null } }];
    ...
    else if (query.includeArchived !== 'true') and.push({ archivedAt: null });
  ...
  private scopeWhere(user: CurrentUserPayload): Prisma.OpportunityWhereInput {
    ...
    if (user.role === UserRole.MANAGER) return user.team ? { company: { owner: { team: user.team } } } : { id: { in: [] } };
    return { OR: [{ ownerId: user.userId }, { company: { ownerId: user.userId } }] };
  }

  private async getForMutation(id: string, user: CurrentUserPayload) {
    if (user.role === UserRole.BOARDS) throw new ForbiddenException('Opportunity is read-only for this role');
  ...
  private async getCompanyInScope(companyId: string, user: CurrentUserPayload) {
    ...
    if (!company || company.archivedAt) throw new NotFoundException('Company not found');
    if (user.role === UserRole.ADMIN) return company;
    if (user.role === UserRole.MANAGER && user.team && company.owner?.team === user.team) return company;
    if (user.role === UserRole.REP && company.ownerId === user.userId) return company;
    throw new ForbiddenException('You do not have access to this company');
  }

  private async validateOwner(ownerId: string, user: CurrentUserPayload) {
    const owner = await this.prisma.user.findUnique({ where: { id: ownerId } });
    if (!owner || !owner.isActive || (owner.role !== UserRole.REP && owner.role !== UserRole.MANAGER)) {
      throw new BadRequestException('Opportunity owner must be an active REP or MANAGER');
    }
    if (user.role === UserRole.REP && owner.id !== user.userId) throw new ForbiddenException('REP can only assign opportunities to self');
    if (user.role === UserRole.MANAGER && (!user.team || owner.team !== user.team)) throw new ForbiddenException('Owner must belong to the manager team');
  }
